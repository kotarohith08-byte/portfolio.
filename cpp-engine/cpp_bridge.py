...
class CPPEngineBridge:
    ...
    def compute_study_roadmap(self, topics: List[Dict[str, Any]], prerequisites: List[Tuple[str, str]]) -> List[str]:
        """
        Topological sort on DAG of topics.
        prerequisites is a list of (prerequisite_id, dependent_topic_id).
        """
        ...
        # If cyclic dependency exists, append remaining topics gracefully
    ...
    def calculate_sm2_review(self, repetitions: int, ease_factor: float, interval_days: int, quality_grade: int) -> Dict[str, Any]:
        ...
        quality_grade: 0 (blackout) to 5 (perfect recall).
        """
    ...
    def optimize_study_session(self, candidates: List[Dict[str, Any]], max_available_minutes: int) -> Dict[str, Any]:
        """
        0/1 Knapsack dynamic programming optimizer to maximize study value under time budget.
        candidates item format: {"topic_id": str, "title": str, "time_cost_minutes": int, "priority_value": int}
        """
        n = len(candidates)
